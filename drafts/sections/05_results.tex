...
This chapter presents the experimental results, including classification performance, feature importance analysis, and comparison between speech tasks.
...
Interview & Random Forest & \textbf{87.0\%} $\pm$ 4.8\% & \textbf{0.88} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Interview task:} 82.0--87.0\% accuracy
\end{itemize}
...
\begin{itemize}
    ...
    \item Interview: RF 87.0\% vs SVM 82.0\% (+5.0\%)
\end{itemize}
...
These results compare favourably with published work on the ANDROIDS corpus. Previous studies using similar methods report accuracies ranging from 65--85\% depending on task and methodology. The 87\% accuracy achieved here on interview speech places this approach among the better-performing methods, while maintaining full interpretability.
...
Figure \ref{fig:feature-importance} shows the feature importance distributions for both tasks.

\begin{figure}[h]
    \centering
    % Note: Include actual figure files
    % \includegraphics[width=0.48\textwidth]{figures/reading_feature_importance.png}
    % \includegraphics[width=0.48\textwidth]{figures/interview_feature_importance.png}
    \caption{Top 20 features by Gini importance: (a) Reading task, (b) Interview task}
...
Cross-validation provides estimates of variability. The standard deviations of approximately 5--7\% indicate that results are reasonably stable across folds. The 15-percentage-point gap between reading and interview performance substantially exceeds the uncertainty bounds, providing confidence that this difference is genuine.

\section{Summary of Findings}

\begin{enumerate}
    \item \textbf{Interview speech is more informative:} 87\% accuracy vs 72\% for reading
    ...
    \end{itemize}
    \item \textbf{Variability measures are key for interview speech:} The most predictive interview features are \textit{standard deviations}, not means
    \item \textbf{Pausing behaviour is highly informative:} Unvoiced segment features rank prominently for interview speech
    \item \textbf{Both classifiers perform well:} Random Forest slightly better, especially on interview data
\end{enumerate}
