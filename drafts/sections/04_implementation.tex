...
For SVM classification, scikit-learn's \texttt{Pipeline} object was used to chain feature standardisation with the classifier. This is not a convenience, it ensures that standardisation parameters (mean and variance) are computed only on training data within each cross-validation fold, preventing information leakage from test samples into the scaling step.
...
\paragraph{Confusion matrices} were generated from cross-validated predictions (using \texttt{cross\_val\_\allowbreak{}predict}), providing error breakdowns without optimistic bias from training-set evaluation.
